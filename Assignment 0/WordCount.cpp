...
void getWcResult(int argc, const char* argv[]){
	...
	for (int i = 0; i < cmd.fileCount; ++i) {
		...
		if( optionsCode & (1<<4) )	printf("L: %d  ", wc.lineCount);
		if( optionsCode & (1<<3) )	printf("W: %d  ", wc.wordCount);
		if( optionsCode & (1<<2) )	printf("C: %d  ", wc.charCount);
		if( optionsCode & (1<<1) )	printf("B: %d  ", wc.byteCount);
		if( optionsCode &  1	 )	printf("LL:%ld  ", wc.longestLineLength);;
		printf(" %s\n", cmd.fileName[i]);
	}
	#else
	for (int i = 0; i < cmd.fileCount; ++i) {
		WcResult wc = getWcResultForFile(cmd.fileName[i], optionsCode);
		if( optionsCode & (1<<4) )	printf(" %d", wc.lineCount);
		if( optionsCode & (1<<3) )	printf(" %d", wc.wordCount);
		if( optionsCode & (1<<2) )	printf(" %d", wc.charCount);
		if( optionsCode & (1<<1) )	printf(" %d", wc.byteCount);
		if( optionsCode &  1	 )	printf(" %ld", wc.longestLineLength);
		printf(" %s\n", cmd.fileName[i]);
	}
	#endif
}
